...
export const WebFallbackMapView = forwardRef<WebFallbackHandle, WebFallbackMapViewProps>(({
  ...
  const preparedAircrafts = useMemo(() => {
    console.log('[WebFallbackMapView] Preparing aircrafts:', {
      totalAircrafts: aircrafts?.length || 0,
      aircraftIds: aircrafts?.map((a: AircraftType) => a.aircraftId) || [],
      aircraftTasks: aircraftTasks?.length || 0
    });
    return (aircrafts || [])
      .map((a: AircraftType) => {
        ...
        console.log('[WebFallbackMapView] Processing aircraft:', {
          aircraftId: a.aircraftId,
          hasCoordinates: !!a.coordinates?.length,
          lastCoordinate: last
        });
        if (!last) return null;
  ...
  useEffect(() => {
    console.log('[WebFallbackMapView] preparedAircrafts updated:', {
      count: preparedAircrafts.length,
      aircrafts: preparedAircrafts.map(a => ({ id: a.aircraftId, color: a.color, last: a.last }))
    });
  }, [preparedAircrafts]);
